...
switch($_GET[act]){
  ...
  default:
    echo "<h2>potensi</h2>
          ...
          <input type=hidden name=module value=potensi>
          <div id=paging>Masukkan Judul potensi : <input type=text name='kata'> <input type=submit value=Cari></div>
          </form>
          <input type=button value='Tambah potensi' onclick=\"window.location.href='?module=potensi&act=tambahpotensi';\">";
    ...
    if ($_SESSION[leveluser]=='admin'){
      $tampil = mysql_query("SELECT * FROM potensi ORDER BY id_berita DESC LIMIT $posisi,$batas");
    }
    else{
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
                <td>$r[judul]</td>
                <td>$tgl_posting</td>
		            <td><a href=?module=potensi&act=editpotensi&id=$r[id_berita]>Edit</a> | 
		                <a href=\"$aksi?module=potensi&act=hapus&id=$r[id_berita]&namafile=$r[gambar]\" onClick=\"return confirm('Apakah Anda benar-benar mau menghapusnya?')\">Hapus</a></td>
		        </tr>";
      $no++;
    }
    echo "</table>";
    ...
    if ($_SESSION[leveluser]=='admin'){
      $tampil = mysql_query("SELECT * FROM potensi WHERE judul LIKE '%$_GET[kata]%' ORDER BY id_berita DESC LIMIT $posisi,$batas");
    }
    else{
      $tampil=mysql_query("SELECT * FROM potensi 
                           WHERE username='$_SESSION[namauser]'
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
                ...
                <td>$tgl_posting</td>
		            <td><a href=?module=potensi&act=editpotensi&id=$r[id_berita]>Edit</a> | 
		                <a href='$aksi?module=potensi&act=hapus&id=$r[id_berita]&namafile=$r[gambar]'>Hapus</a></td>
		        </tr>";
      $no++;
    }
    echo "</table>";
    ...
    if ($_SESSION[leveluser]=='admin'){
      $jmldata = mysql_num_rows(mysql_query("SELECT * FROM potensi WHERE judul LIKE '%$_GET[kata]%'"));
    }
    else{
      $jmldata = mysql_num_rows(mysql_query("SELECT * FROM potensi WHERE username='$_SESSION[namauser]' AND judul LIKE '%$_GET[kata]%'"));
    }  
    $jmlhalaman  = $p->jumlahHalaman($jmldata, $batas);
    $linkHalaman = $p->navHalaman($_GET[halaman], $jmlhalaman);
  ...
  case "tambahpotensi":
    echo "<h2>Tambah potensi</h2>
          ...
          <select name='kategori'>
            ...
            $tampil=mysql_query("SELECT * FROM katpo ORDER BY nama_kategori");
            while($r=mysql_fetch_array($tampil)){
    ...
    echo "</select></td></tr>
           <tr><td>Headline</td>    <td> : <input type=radio name='headline' value='Y' checked>Y  
                                         ...
                                         <br>(Apabila berita potensi ada gambarnya dan ingin dijadikan headline, pilih Headline = Y)</td></tr>
          <tr><td>Isi Berita</td>  <td> <textarea name='isi_berita' id='editor1' style='width: 600px; height: 350px;'></textarea>
		  <script>
  ...
  case "editpotensi":
    if ($_SESSION[leveluser]=='admin'){
      $edit = mysql_query("SELECT * FROM potensi WHERE id_berita='$_GET[id]'");
    }
    else{
      $edit = mysql_query("SELECT * FROM potensi WHERE id_berita='$_GET[id]' AND username='$_SESSION[namauser]'");
    }
   ...
   if ($r[headline]=='Y'){
      ...
      echo "<tr><td>Isi Berita</td>   <td> <textarea id='editor1' name='isi_berita' style='width: 600px; height: 350px;'>$r[isi_berita]</textarea>
	  <script>
