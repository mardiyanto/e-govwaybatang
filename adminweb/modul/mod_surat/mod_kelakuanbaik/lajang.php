...
$aksi="modul/mod_surat/mod_kelakuanbaik/aksi_lajang.php";
switch($_GET[act]){
  // Tampil Modul
  default:
...
<div class='row'>
   ...
   <h3>Permohonan Surat Kelakuan Baik</h3>
</div>
                                
                                <div class='panel-body'>
		  ";
	 $cek_username=mysql_num_rows(mysql_query
             ("SELECT * FROM srt_k_baik"));
...
if ($cek_username <= 0){
  echo "<h3>Data Masih Kosong</h3>";
	...
	<tbody>";
	$no=1;
    $tampil=mysql_query("SELECT * FROM srt_k_baik ORDER BY id_srt_d ASC");
...
echo"<a href=media.php?module=kalakuanbaik&act=lihat&id=$r[id_srt_d]>lihat</a>";
					...
					
					{
						...
						echo"<a href='cetak/cetak_k_kelakuanbaik.php?id_srt_d=$r[id_srt_d]' target='_blank'  onclick='window.open ('cetak/cetak_k_kelakuanbaik.php?id_srt_d=$r[id_srt_d]','myWin','resizable=1,status=1,menubar=1,toolbar=1,
	    ...
	    <a href=media.php?module=kalakuanbaik&act=edit&id=$r[id_srt_d]>edit</a>
		    |
	    
	    <a href=$aksi?module=kalakuanbaik&act=hapus&id=$r[id_srt_d]>hapus</a>
            </td>
...
break;
  ...
  case "lihat":
        $edit=mysql_query("SELECT * FROM srt_k_baik WHERE id_srt_d='$_GET[id]'");
...
<div class='row'>
  ...
  Lihat Surat
   ...
   <form method=POST action=$aksi?module=kalakuanbaik&act=update>
 ...
 <p class=inline-small-label> 
   ...
   <input type='text' class='form-control' placeholder='isi keterangan pengambilan surat' name=keter  size=40>
   </p>
...
<br/><br/><div class=block-actions> 
      ...
      <a class='btn btn-primary btn-sm' red' id=reset-validate-form href='?module=kalakuanbaik'>Batal</a>
...
<div class='row'>
  ...
  edit Surat
	 ...
	 <div class='panel-body'>	
                  ...
                  <form method=POST action=$aksi?module=kalakuanbaik&act=edit>
   <input type=hidden name=id value='$hasil[id_srt_d]'>
	  ...
	  <label for='email'>Status</label>	<select class='form-control select2' name='status' id='status' >
